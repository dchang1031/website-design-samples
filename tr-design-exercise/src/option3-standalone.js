(() => {
  ...
  function update(){
    ...
    // Keep the headline centered and fully opaque through the reveal. It only
    // disappears after every character has reached full opacity.
    const fade=smooth(.48,.68,p);
    headline.style.opacity=String(1-fade);
    headline.style.filter=`blur(${(fade*8).toFixed(2)}px)`;

    // The unified access card arrives first; the supporting copy follows.
